...
def calculate_checksum(box_ids):
    id_counts = [Counter(box_id).values() for box_id in box_ids]
    twos = [1 for counts in id_counts if 2 in counts]
    threes = [1 for counts in id_counts if 3 in counts]
    checksum = sum(twos) * sum(threes)
    return checksum

def find_common_letters(box_ids):
    for box_1_i, box_1_id in enumerate(box_ids):
        for box_2_i, box_2_id in enumerate(box_ids[box_1_i+1:]):
            diff_pos = [i
                        for i, (char_1, char_2) in enumerate(zip(box_1_id,
                                                                 box_2_id))
                        if char_1 != char_2]
            if len(diff_pos) == 1:
                return box_1_id[:diff_pos[0]] + box_1_id[diff_pos[0]+1:]

if __name__ == '__main__':
    box_ids = []
    with open('day_2_box_ids.txt', 'r') as box_ids_file:
        box_ids = box_ids_file.read().splitlines()

    print("Checksum:", calculate_checksum(box_ids))
    print("Common letters:", find_common_letters(box_ids))
