...
"I don't have time to draw out a map of this place - it's huge. Instead,
I can give you directions to every room in the facility!" He writes down some
directions on a piece of parchment and runs off. In the example above,
the instructions might have been ^WNE$, a regular expression or "regex"
(your puzzle input).

The regex matches routes (like WNE for "west, north, east") that will take you
from your current room through various doors in the facility. In aggregate,
...
a branch: after going north, you must choose to go either east or west before
...
def room_distances(directions):
    current_room = (0, 0)
    distances = {current_room: 0}
    number_of_doors = 0
    junctions = []
    for step in directions[1:-1]:
        if step == '(':
            junctions.append((current_room, number_of_doors))
        elif step == ')':
            current_room, number_of_doors = junctions.pop()
        elif step == '|':
            current_room, number_of_doors = junctions[-1]
        else:
            if step == 'N':
            ...
            if current_room in distances:
                if distances[current_room] > number_of_doors:
                    distances[current_room] = number_of_doors
            else:
                distances[current_room] = number_of_doors
    return distances

def min_doors_to_furthest_room(directions):
    distances = room_distances(directions)
    return max(distances.values())

def rooms_min_doors(directions, min_number_of_doors=1000):
    distances = room_distances(directions)
    return len({room: doors for room, doors in distances.items()
                            if doors >= min_number_of_doors})

def main():
    directions = {}
    with open('day_20_regex_directions.txt', 'r') as regex_directions_file:
        directions = regex_directions_file.read().strip()

    print("Minimum number of doors to farthest room:",
        min_doors_to_furthest_room(directions))

    min_number_of_doors = 1000
    print(f"Rooms at least {min_number_of_doors} doors away:",
        rooms_min_doors(directions, min_number_of_doors=min_number_of_doors))
