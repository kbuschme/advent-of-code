...
The Elves are making decent progress, but are still struggling with the suit's
...
def shortest_polymer_minus_unit(polymer):
    units = set(polymer.lower())
    resulting_lengths = {}
    for unit in units:
        cleaned_polymer = polymer.replace(unit, "").replace(unit.upper(), "")
        resulting_lengths[unit] = len(react(cleaned_polymer))
    return min(resulting_lengths.items(), key=lambda p: p[1])[1]

def main():
    polymer = ""
    with open("day_5_polymere.txt", 'r') as polymer_file:
        polymer = polymer_file.read().splitlines()[0]

    print(f"Polymer length (after reaction): {len(react(polymer))}")
    shortest_polymer = shortest_polymer_minus_unit(polymer)
    print(f"Polymer length (after removing one unit): {shortest_polymer}")
