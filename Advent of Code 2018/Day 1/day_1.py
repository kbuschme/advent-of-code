...
- "that will let you fix the changes with no such propagation delay.
...
duplicate frequency is found, and that duplicates might be found while
...
def find_frequency_sum(freq_changes):
    return sum(freq_changes)

def find_repeated_frequency(freq_changes):
    index = 0
    max_index = len(freq_changes)
    tally = 0
    sums = set()
    repeat = True
    while repeat:
        tally = tally + freq_changes[index]
        if tally in sums:
            return tally
        else:
            sums.add(tally)
...
def main():
    freq_changes = []
    with open("day_1_frequency_changes.txt", 'r') as chanages_file:
        freq_changes = [int(change)
                        for change in chanages_file.read().splitlines()]

    print("Resulting frequency:", find_frequency_sum(freq_changes))
    print("Frequency found twice:", find_repeated_frequency(freq_changes))
