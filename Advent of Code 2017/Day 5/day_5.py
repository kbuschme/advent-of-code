...
one line, with the current instruction marked in parentheses. The following
...
Your puzzle answer was 25558839.
"""

def number_of_instruction_steps(instructions):
    steps = 0
    index = 0
...
def number_of_instruction_steps_strange(instructions):
    steps = 0
    index = 0
    last_index = len(instructions)
    while index < last_index:
...
def main():
    filename = 'day_5_instructions.txt'
    with open(filename, 'r') as instructions_file:
        instructions = instructions_file.readlines()
    instructions = [int(each) for each in instructions]

    print("Number of instructions:", len(instructions))
    print("Number of steps:", number_of_instruction_steps(instructions[:]))
    print("Number of steps (strange):",
          number_of_instruction_steps_strange(instructions[:]))
