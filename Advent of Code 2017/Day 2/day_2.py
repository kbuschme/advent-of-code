...
def max_min_difference_checksum(spreadsheet):
    return sum([max(row) - min(row) for row in spreadsheet])

def evenly_divisible_checksum(spreadsheet):
    return sum([dividend//divisor for row in spreadsheet
                                  for index, divisor in enumerate(row)
                                  for dividend in row[:index] + row[index+1:]
                                  if dividend % divisor == 0])

def main():
    filename = 'day_2_spreadsheet.txt'
    with open(filename, 'r') as spreadsheet_file:
        spreadsheet = spreadsheet_file.read().strip().split('\n')
    spreadsheet = [[int(number) for number in row.split('\t')]
                   for row in spreadsheet]

    print("Max-min difference checksum:",
        max_min_difference_checksum(spreadsheet))
    print("Evenly-divisible checksum:",
        evenly_divisible_checksum(spreadsheet))
