...
def generate_multiple(previous_value, factor, multiple):
    divisor = 2147483647
    next_value = (previous_value * factor) % divisor
    while not (next_value % multiple == 0):
        next_value = (next_value * factor) % divisor
    return next_value

def count_matches(starting_values, factors, trials=40_000_000, multiples=None):
    matches = 0
    previous_values = starting_values
    for i in range(trials):
        if multiples is None:
            previous_values['A'] = generate(previous_values['A'], factors['A'])
            previous_values['B'] = generate(previous_values['B'], factors['B'])
        else:
            previous_values['A'] = generate_multiple(
                previous_values['A'], factors['A'], multiples['A'])
            previous_values['B'] = generate_multiple(
                previous_values['B'], factors['B'], multiples['B'])

        if bin(previous_values['A'])[-16:] == bin(previous_values['B'])[-16:]:
        # if (previous_values['A'] & 0xFFFF) == (previous_values['B'] & 0xFFFF):
            matches = matches + 1

    return matches

def main():
    generator_factors = {'A': 16807, 'B': 48271}
    starting_values = {'A': 703, 'B': 516}
    multiples = {'A': 4 ,'B': 8}
    print("Matches:", count_matches(starting_values.copy(), generator_factors))
    print("Matches (multiples):",
          count_matches(starting_values.copy(), generator_factors,
                        trials=5_000_000, multiples=multiples))
