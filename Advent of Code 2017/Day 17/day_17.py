...
def fill_buffer(steps, max_value=2017, verbose=False):
    circular_buffer = [0]
    current_index = 0
    for new_value in range(1, (max_value+1)):
        ...
        next_index = ((current_index + steps) % buffer_length) + 1
        circular_buffer.insert(next_index, new_value)
        current_index = next_index
        if verbose:
            print(current_index, circular_buffer)
...
def find_value_after_last_insert(steps, max_value):
    circular_buffer = fill_buffer(steps, max_value=max_value)
    return circular_buffer[circular_buffer.index(max_value) + 1]

def find_value_after_zero_brute_force(steps, max_value):
    # This is way too slow for my machine :(
    circular_buffer = fill_buffer(steps, max_value=max_value)
    return circular_buffer[circular_buffer.index(0) + 1]

def find_value_after_zero(steps, max_value, verbose=False):
    zero_index = 0
    buffer_length = 1
    number_after_zero = None

    if verbose:
        print("\nSteps:", steps, "max_value:", max_value)

    current_index = 0
    for new_value in range(1, (max_value+1)):
        if verbose and (new_value % 1_000_000 == 0):
            print(new_value)

        next_index = ((current_index + steps) % buffer_length)
        if next_index == zero_index:
            number_after_zero = new_value
            if verbose:
                print(new_value)
...
def main():
    ...
    print("Value after last insertion:",
          find_value_after_last_insert(steps, max_value=2017))
    print("Value after zero:",
          find_value_after_zero(steps, max_value=50_000_000))
