...
directly or indirectly. The programs you identified just a moment ago are
...
def count_group_members(pipe_list, target_id=0):
    pipe_list = [row.split(' <-> ') for row in pipe_list]
    address_book = {int(program): [int(c) for c in contacts.split(', ')]
                    for program, contacts in pipe_list}
    look_up = {target_id}
    group_members = set()
    ...
    while len(look_up) > 0:
        ...
        new_contacts = {contact
                        for contact in address_book[program]
                        if contact not in group_members}
        look_up.update(new_contacts)
        group_members.add(program)

    return group_members

def count_groups(pipe_list):
    programs = [int(row.split(' <-> ')[0]) for row in pipe_list]
    groups = []
    for program in programs:
        ...
        for group in groups:
            if program in group:
                in_group = True
        if not in_group:
            groups.append(count_group_members(pipe_list, target_id=program))

    return len(groups)

def main():
    filename = 'day_12_pipe_list.txt'
    with open(filename, 'r') as pipe_file:
        pipe_list = pipe_file.readlines()
    print("Programs in group with program ID 0:",
        len(count_group_members(pipe_list)))
    print("Groups:", count_groups(pipe_list))
