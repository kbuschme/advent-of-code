...
Your puzzle answer was 3966414.
"""

def calculate_severity(firewall, delay=0, verbose=False):
    last_layer = max(firewall.keys())
    severities = []
    for time in range(last_layer + 1):
        if time in firewall:
            layer_range = firewall[time]
            if verbose:
                print("Range:", layer_range)
                print("Time:", time)
            steps_to_origin = (layer_range-1) * 2
            if (time % steps_to_origin) == 0:
                severities.append(time*layer_range)
    if verbose:
        print("Severities:", severities)

    return sum(severities)

def calculate_delay(firewall):
    def caught(firewall, delay=0, verbose=False):
        last_layer = max(firewall.keys())
        severities = []
        if verbose:
            print("Delay:", delay)
        for time in range(last_layer + 1):
            if time in firewall:
                ...
                if verbose:
                    ...
                    print("Time:", time)
                steps_to_origin = (layer_range-1) * 2
                if ((time+delay) % steps_to_origin) == 0:
                    return True
        return False
    ...
    while not passed:
        if caught(firewall, delay=delay):
            delay = delay + 1
        else:
            return delay

def main():
    filename = 'day_13_firewall.txt'
    with open(filename, 'r') as firewall_file:
        firewall = [row.split(': ') for row in firewall_file.readlines()]
    firewall = {int(layer_depth): int(layer_range)
                for layer_depth, layer_range in firewall}
    print("Severity:", calculate_severity(firewall))
    print("Delay:", calculate_delay(firewall))
